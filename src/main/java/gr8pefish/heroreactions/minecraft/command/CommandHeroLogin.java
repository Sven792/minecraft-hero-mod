    ...
    public void execute(MinecraftServer server, ICommandSender sender, String[] params) throws CommandException {
        if (params.length > 0 && params.length <= 2) {
            ...
            if (params[0].equalsIgnoreCase(LOGIN)) {
                ...
                TextComponentString postLink = new TextComponentString(", then paste the token you get here by running '/hero login [token_here]'"); //TODO: localize;
                ...
                if (params.length == 2) { //message supplied (contained in params[1])
                    if (params[1].equalsIgnoreCase("help")) {
                        ...
                        //token login
                        Common.LOGGER.info("Use token " + params[1]); //TODO
                    }
                } else { //no message
                    sender.sendMessage(helpMsg);
                }
